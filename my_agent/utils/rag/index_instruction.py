...
class IndexConstructionModule:
    ...
    def load_index(self):
        ...
        if not Path(self.index_save_path).exists():
            logger.info(f"索引路径不存在: {self.index_save_path}，无法加载索引")
            return None
        ...
        try:
            ...
            logger.warning(f"加载向量索引失败: {e}, 请检查索引文件是否完整或重新构建索引")
            return None
    
    def index_similarity_search(self, query: str, k: int = 5) -> List[Document]:
        """
        相似度搜索
