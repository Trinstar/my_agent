...
class DataPreparationModule:
    ...
    def __init__(self, data_path: str):
        ...
        self.chunks_path = data_path + "/chunks.pkl"
        self.documents_path = data_path + "/documents.pkl"
        self.documents: List[Document] = []  # 父文档（完整食谱）
        self.chunks: List[Document] = []     # 子文档（按标题分割的小块）
        self.parent_child_map: Dict[str, str] = {}  # 子块ID -> 父文档ID的映射
    
    @classmethod
    def get_supported_categories(cls) -> tuple[str, ...]:
        """对外提供支持的分类标签列表"""
        return tuple(cls.CATEGORY_LABELS)

    @classmethod
    def get_supported_difficulties(cls) -> tuple[str, ...]:
        """对外提供支持的难度标签列表"""
        return tuple(cls.DIFFICULTY_LABELS)
    ...
    def load_documents(self) -> List[Document]:
        ...
        if os.path.exists(self.documents_path):
            ...
            logger.info(f"已从缓存加载 {len(self.documents)} 个文档")
        ...
        for md_file in data_path_obj.rglob("*.md"):
            try:
                ...
                try:
                    ...
                    absolute_path = Path(md_file).resolve()
        ...
        self.save_docs_and_chunks()
        logger.info(f"文档数据已保存到{self.documents_path}")
        return documents
    ...
    def chunk_documents(self) -> List[Document]:
        ...
        if os.path.exists(self.chunks_path):
            ...
            logger.info(f"已从缓存加载 {len(self.chunks)} 个分块")
            return self.chunks
        ...
        self.save_docs_and_chunks()
        logger.info(f"分块数据已保存{self.chunks_path}")
        return chunks
    ...
    def get_parent_documents(self, child_chunks: List[Document], parent_documents: List[Document]) -> List[Document]:
        """
        根据子块获取对应的父文档（智能去重）
    ...
    def save_docs_and_chunks(self):
        import pickle
        if self.chunks and not os.path.exists(self.chunks_path):
            with open(self.chunks_path, 'wb') as f:
                pickle.dump(self.chunks, f)
            logger.info(f"分块数据已保存到: {self.chunks_path}")
        if self.documents and not os.path.exists(self.documents_path):
